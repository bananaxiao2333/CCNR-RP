...
当前版本（`2.20.3`）已实现 **P0–P9 全流程闭环**：事件 → 刷新波/招募 → 部署 → 判死 → 冷却 → 复活 → 结算。
...
1. 使用 Forge `47.2.0+`（Minecraft **1.20.1**）启动器（如 HMCL / 官方启动器），把 `ccnr_rp-2.20.3.jar` 放入 `mods` 目录。
...
产物：`build/libs/ccnr_rp-2.20.3.jar`。工程规范见 [docs/01](docs/01-工程规范.md)，开发注意见 [AGENTS.md](AGENTS.md)。

...
当前版本（`2.24.0`）已实现 **P0–P9 全流程闭环**：事件 → 刷新波/招募 → 部署 → 判死 → 冷却 → 复活 → 结算。
...
| P16 属性与区域 | 阵营属性（含 FirstAid 解耦适配）+ 区域（拓展设定） | [docs/16](docs/16-属性与区域.md) |
...
- **区域与外部功能接口（2.24.0）**：「拓展设定」页签定义目标区，外部功能（如核弹，非本 mod 内容）按 id 读取区域与阵营弹头许可，依赖方向单向（外部 → 本 mod）。
- 支持中英文界面语言（游戏语言自动切换）。

## 安装（客户端 + 服务端）

1. 使用 Forge `47.2.0+`（Minecraft **1.20.1**）启动器（如 HMCL / 官方启动器），把 `ccnr_rp-2.24.0.jar` 放入 `mods` 目录。
2. 首次启动自动生成默认配置：`config/ccnr_rp/` 下的 `factions.json` / `phases.json` / `events.json` / `animations.json` / `spawn_waves.json`（含精简样板：3 个样例阵营、4 个样例职业）。
...
产物：`build/libs/ccnr_rp-2.24.0.jar`。工程规范见 [docs/01](docs/01-工程规范.md)，开发注意见 [AGENTS.md](AGENTS.md)。

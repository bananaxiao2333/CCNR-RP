...
5. 招募（RecruitManager）：对每个邀请发 RecruitOfferS2C（offerId、groupId、残余倒计时、职业/阵营预览），
   客户端**屏幕右侧**渲染招募列表（立绘/职业/倒计时条/接受/拒绝）。**已加入名单实时同步**：accept 登记已加入
   并向该波全部候选推送 `RecruitRosterS2C`（groupId + 展示 id + 目标数 + 已加入条目，服务端权威整表），
   客户端在**背包右上角**绘制「已加入玩家」（头部显示已加入 x/需要 y，立绘取对应玩家本人皮肤）；有人接受/离服
   （取消接受）/结算时增量重推，结算/取消时推空名单清除，客户端登出整清。人满即提前结算；超时按已加入部署；
   拒绝/超时清理征召登记；全员无响应→公告"波次招募失败"。

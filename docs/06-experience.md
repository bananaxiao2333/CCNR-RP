...
## 6. 结算（瞬时、服务端权威；触发点保持 v2）

- 触发：`/rp settle [player|all]`、退场结算（死亡/判死/退役（SKIP_SETTLE 除外）/
  征召结束）、事件结束（settleOnEnd）、游戏结束全员。
...
- 底部一行 = **白色经验数字**（当前总经验）；其上 = **经验变化项目列**
  （正=绿、负=红、带 `+/−` 符号，标题+数值）。
- **结算动画**（纯视觉；服务端结算瞬时完成）：最底一项移入数字并消失 →
  数字更新 → 列表下移补齐 → 循环至全部吸入 → 数字停在新总值。

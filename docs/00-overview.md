...
| `variable` | 自定义设定（`VariableType`/`Variable`/`VariableRegistry` 纯逻辑 + `VariableService` 配置读写），外部功能的只读取值接口 | P17 |
...
| `config/ccnr_rp/factions.json` | 阵营 + 阵营组 + 关系 + 职业 + 装备 Loadout + 阵营属性（docs/16） | 是 |
| `config/ccnr_rp/variables.json` | 自定义设定：全局变量（bool/number/text）+ 每变量预设值 + 整套预设方案（docs/17） | 是 |
...
> P16 玩家属性（[docs/16](docs/16-玩家属性.md)，2.24.0）、
> P17 自定义设定/全局变量（[docs/17](docs/17-自定义设定.md)，2.25.0）。
...
`/rp attribute …`（阵营属性）、`/rp var …`（自定义设定：变量/预设值/预设方案）。
